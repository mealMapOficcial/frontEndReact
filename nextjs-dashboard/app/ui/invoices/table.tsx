...
export default function InvoicesTable() {
  ...
  ]; // Datos ficticios para fines de diseño

  return (
    <div className="mt-6 flow-root">
      <div className="inline-block min-w-full align-middle">
        <div className="rounded-lg bg-orange-500 p-2 md:pt-0">
            ...
            {invoices.map((invoice) => (
              <div key={invoice.id} className="mb-2 w-full rounded-md bg-white p-4">
                <div className="flex items-center justify-between border-b pb-4">
                  ...
                  {/* Aquí se pasa el status correctamente */}
                  <InvoiceStatus status={invoice.status} />
                </div>
                <div className="flex w-full items-center justify-between pt-4">
                  <div>
                    <p className="text-xl font-medium">$500.00</p>
            ...
            <tbody className="bg-white">
              {invoices.map((invoice) => (
                <tr key={invoice.id} className="w-full border-b py-3 text-sm">
                  ...
                  <td className="whitespace-nowrap px-3 py-3">$500.00</td>
                  <td className="whitespace-nowrap px-3 py-3">{new Date().toLocaleDateString()}</td>
                  {/* Aquí también se pasa el status correctamente */}
